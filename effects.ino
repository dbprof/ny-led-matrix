...
// взрывчики
void explosionRoutine() {
  //modeCode = 13;
  for (byte i = 0; i < 1; i++) {
    byte x = random(0, WIDTH);
    byte y = random(0, HEIGHT);
    int iCol = random(0, 255);
    if (getPixColorXY(x, y) == 0) leds[getPixelNumber(x, y)] = CHSV(iCol, 255, 255);
    //if (x+1<=WIDTH && y+1<=HEIGHT) leds[getPixelNumber(x+1, y+1)] = CHSV(iCol, 150, 150);
    //if (x-1>=0 && y+1<=HEIGHT) leds[getPixelNumber(x-1, y+1)] = CHSV(iCol, 150, 150);
    //if (x+1<=WIDTH && y-1>=0) leds[getPixelNumber(x+1, y-1)] = CHSV(iCol, 150, 150);
    //if (x-1>=0 && y-1>=0) leds[getPixelNumber(x-1, y-1)] = CHSV(iCol, 150, 150);
  }
  fader(70);
}
